...
$ToplamKayitSayisiSorgusu=mysqli_query($link,"SELECT * FROM favoriler WHERE UyeId = '22' ORDER BY id DESC");
$ToplamKayitSayisiSorgusu=mysqli_num_rows($ToplamKayitSayisiSorgusu);
	...
	<tr>
		<td width="1065" valign="top">
			<table width="1065" align="center" border="0" cellpadding="0" cellspacing="0">
				...
				$FavorilerSorgusu=mysqli_query($link,"SELECT * FROM favoriler WHERE UyeId = '$KullaniciID' ORDER BY id DESC LIMIT $SayfalamayaBaslanacakKayitSayisi, $SayfaBasinaGosterilecekKayitSayisi");
				$FavoriSayisi=mysqli_num_rows($FavorilerSorgusu);

				if($FavoriSayisi>0){
					while($FavoriKayitlari = $FavoriSatirlar->fetch_assoc()){
						...
						$FavoriSatirlar=$FavoriSatirlar["UrunId"];

						$UrunlerSorgusu=mysqli_query($link,"SELECT * FROM urunler WHERE id = '$FavoriSatirlar' LIMIT 1");
